...
The causal credibility of this identification strategy rests on four key sources. First, the 2004 federal minimum wage increase created exogenous policy variation—a natural experiment that triggered staggered state responses largely unrelated to contemporaneous local labor market conditions. This federal shock provides plausibly exogenous variation that separates minimum wage policy decisions from underlying employment trends affecting teen workers. Second, the parallel trends assumption is empirically validated through pre-treatment analysis showing flat effects in periods before minimum wage adoption, indicating that treated and untreated counties followed similar employment trajectories prior to policy changes. Third, the study employs modern DiD methods that explicitly address heterogeneous treatment effect bias—a critical innovation that solves the negative weighting problems plaguing traditional two-way fixed effects estimators when effects vary across groups and time. Fourth, results are robust across multiple specifications, including both unconditional and conditional DiD models with covariate adjustment. This combination of exogenous variation, validated assumptions, modern estimation methods, and specification robustness provides strong causal identification for the estimated employment effects.
...
The study finds that minimum wage increases generally reduce teen employment. Table 3 presents specific ATT(g,t) estimates by treatment cohort and time period. The 2004 cohort shows the largest negative effects: conditional estimates indicate teen employment declines by approximately 8.9 log points in the first year after minimum wage increases (2004→2005), 12.1 log points in the second year (2005→2006), and 18.3 log points by the third year (2006→2007). The 2006 cohort shows a positive pre-treatment effect of 6.9 log points in 2004, but transitions to negative effects of -2.5 log points in the post-treatment period. The 2007 cohort shows small negative effects in the post-treatment period. These estimates are statistically significant for the 2004 cohort across all post-treatment periods \citep{callaway2021difference}.

The negative effects exhibit clear dynamics, growing larger several years after the minimum wage increase, indicating that the impact of the policy intensifies with longer exposure. The 2004 cohort's effects expand from -0.089 to -0.183 over three years, suggesting that firms may initially absorb higher labor costs but gradually reduce employment through reduced hiring or changes in work schedules as they adapt to the new wage floor. The results are robust across multiple specifications: the close similarity between unconditional and conditional DiD estimates (differences of less than 0.02 log points) indicates that controlling for demographic and economic covariates does not substantially alter the estimated effects. This robustness is further confirmed through doubly robust DiD estimators with simultaneous confidence bands, which combine outcome regression with propensity score weighting to protect against model misspecification \citep{callaway2021difference}.
...
Figure 1 presents the dynamic treatment effects for each cohort, showing the evolution of ATT(g,t) estimates over time. Each colored line represents one treatment cohort (2004, 2006, 2007), with shaded areas indicating 95\% confidence intervals. The horizontal dashed line at zero marks the null effect of no treatment impact.

The visualization demonstrates several key patterns. First, pre-treatment periods (left of each cohort's treatment point) show effects clustered around zero, providing visual validation of the parallel trends assumption. Second, negative effects emerge immediately following treatment implementation and generally intensify over time, particularly for the 2004 cohort. Third, the 2004 cohort shows the most dramatic and sustained negative trajectory across the post-treatment years, with effects becoming increasingly negative in the later calendar years. This pattern aligns with economic theory suggesting that firms may initially absorb higher labor costs but gradually reduce employment through reduced hiring or changes in work schedules as they adapt to the new wage floor.
...
Following the same approach as the main analysis, the heterogeneous effects are estimated using log-transformed county teen employment levels, with ATT estimates initially calculated in log points and then converted to percentages for interpretation. This conversion provides economically meaningful effect sizes while maintaining the statistical properties of the DiD estimator.

The analysis reveals important patterns of heterogeneity across treatment cohorts. For the 2004 cohort, high-poverty counties experience a first-year employment decline of 13.2\% while low-poverty counties show a smaller 4.8\%* decline. The 8.4 percentage point difference between groups, while economically meaningful, is not statistically significant due to the wide confidence interval for high-poverty counties. For the 2006 cohort, both groups experience statistically significant employment declines: 7.7\%*** in high-poverty counties and 6.1\%*** in low-poverty counties, indicating minimal heterogeneity for this cohort.
...
The subsample DiD approach estimates separate treatment effects for clearly defined groups by splitting the sample and applying the same DiD estimator to each subsample independently. This method was chosen over alternative approaches like continuous interaction models because it provides transparent and easily interpretable results that policymakers can directly compare across groups. Each subsample estimation can be tailored to the specific characteristics of that group, allowing for different variance structures and relationships with control variables.
